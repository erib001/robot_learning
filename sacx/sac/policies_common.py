...
class MultiHead_ContinuousCritic(BaseModel):
    """
    Critic network(s) for DDPG/SAC/TD3.
    It represents the action-state value function (Q-value function).
    Compared to A2C/PPO critics, this one represents the Q-value
    ...
    :param observation_space: Observation space
    :param action_space: Action space
    :param net_arch_shared: Network architecture for the shared backbone
    :param net_arch_head: Network architecture for different heads
    :param features_extractor: Network to extract features
        (a CNN when using images, a nn.Flatten() layer otherwise)
    :param features_dim: Number of features
    :param activation_fn: Activation function
    ...
    def __init__(
        ...
        action_dim = get_action_dim(self.action_space)
        #maybe need to hardcode
        #n_tasks = self.n_envs
        #self.n_envs = num_tasks #ATTENTION
        self.num_tasks = num_tasks
        self.share_features_extractor = share_features_extractor
        self.n_critics = n_critics
        #self.backbone: list[nn.Module] = []
        self.backbone= nn.ModuleList()
        #Backbone
        for idx in range(n_critics):
            q_net_list = create_mlp(features_dim + action_dim, 512, net_arch, activation_fn)
            q_net = nn.Sequential(*q_net_list)
            #self.add_module(f"qf{idx}", q_net)
            self.backbone.append(q_net)

        #Multihead
        self.critic_heads = nn.ModuleList()
        for idx in range(n_critics):
            #Alternative
            #task_q_networks = nn.ModuleList()
            heads = nn.ModuleList()
            for task_idx in range (num_tasks):
                q_net_list = create_mlp(512, 1, net_arch_head, activation_fn)
                q_net = nn.Sequential(*q_net_list)
                #self.add_module(f"qf_critic{idx}_task_{task_idx}", q_net)
                heads.append(q_net)
            self.critic_heads.append(heads)
    ...
    def forward(self, obs: th.Tensor, actions: th.Tensor) -> tuple[th.Tensor, ...]:
        ...
        with th.set_grad_enabled(not self.share_features_extractor):
            features = self.extract_features(obs, self.features_extractor)
        task_encoding = obs[:,-self.num_tasks:]
        qvalue_input = th.cat([features, actions], dim=1)
        #backbone_output = tuple(q_net(qvalue_input) for q_net in self.q_networks)        
        final_q_values = []
        #print("TAKSLJFLASÖJ\n\n\n")
        for critic_idx in range (self.n_critics):
            backbone_output = self.backbone[critic_idx](qvalue_input)
            #q_output = th.zeros(self.batch_size, 1, device=obs.device)
            q_output = th.zeros(obs.shape[0], 1, device=obs.device)

            for task_idx in range(self.num_tasks):
                mask = task_encoding[:, task_idx].bool()
                head_input = backbone_output[mask]
                q_output[mask] = self.critic_heads[critic_idx][task_idx](head_input)
            final_q_values.append(q_output)
        return tuple(final_q_values)
    
    def forward_old(self, obs: th.Tensor, actions: th.Tensor) -> tuple[th.Tensor, ...]:
        # Learn the features extractor using the policy loss only
        # when the features_extractor is shared with the actor
        with th.set_grad_enabled(not self.share_features_extractor):
            features = self.extract_features(obs, self.features_extractor)
        task_encoding = obs[:,-self.num_tasks:]
        qvalue_input = th.cat([features, actions], dim=1)
        backbone_output = tuple(q_net(qvalue_input) for q_net in self.backbone)
        ...
        for critic_idx in range (self.n_critics):
            ...
            for task_idx in range(self.num_tasks):
                mask = task_encoding[:, task_idx].bool()
                head_input = backbone_output[critic_idx][mask]
                head_output = self.all_networks[critic_idx][task_idx](head_input)
                q_output[mask] = head_output
            final_q_values.append(q_output)
        return tuple(final_q_values)
    ...
    def q1_forward(self, obs: th.Tensor, actions: th.Tensor) -> th.Tensor:
        ...
        with th.no_grad():
            features = self.extract_features(obs, self.features_extractor)
        return self.backbone[0](th.cat([features, actions], dim=1))
